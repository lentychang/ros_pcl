...
void matching_descriptor(std::string& dataDir, std::string& modelName, std::string& detector, std::string& descriptor) {
    ...
    if (detector == "iss" || detector == "sift" || detector == "uni") {
        ...
        if (descriptor == "pfh" || descriptor == "fpfh" || descriptor == "vfh" || descriptor == "rsd" ||
            ...
            modelDescriptorFilePath =
                dataDir + "/descriptor/" + detector + "/" + modelName + "_" + descriptor + "_.pcd";
        }
        else {
            ROS_FATAL("unknown descriptor name, please enter only [pfh|fpfh|vfh|rsd|sc3d|usc|shot]");
        }
    }
    else {
        ROS_FATAL("unknown detector name, please enter only [iss|sift|uni]");
    }
    ...
    for (size_t i = 0; i < sceneDescriptor->size(); ++i) {
        ...
        // Ignore NaNs.
        if (!pcl_isfinite(sceneDescriptor->at(i).descriptor[0])) { continue; }
    ...
    pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> off_scene_model_keypoints_color_handler(
    ...
    viewer.setPointCloudRenderingProperties(
        pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 3, "off_scene_model_keypoints");
...
void matching_histogram(std::string& dataDir, std::string& modelName, std::string& detector, std::string& descriptor) {
    ...
    for (size_t i = 0; i < sceneDescriptor->size(); ++i) {
        ...
        std::vector<float> squaredDistances(1);
        // Ignore NaNs.
        if (!pcl_isfinite(sceneDescriptor->at(i).histogram[0])) { continue; }
        // Find the nearest neighbor (in descriptor space)...
        int neighborCount = matching.nearestKSearch(sceneDescriptor->at(i), 1, neighbors, squaredDistances);
        // ...and add a new correspondence if the distance is less than a threshold
        // (SHOT distances are between 0 and 1, other descriptors use different metrics).
...
bool matchingSrvCb(ros_pcl_msgs::srv_matching::Request& req, ros_pcl_msgs::srv_matching::Response& res) {
    ...
    if (req.descriptor == "pfh") {
        ...
        // matching_histogram<pntType, pntNType, pcl::PrincipalRadiiRSD>(req.dataDir, req.modelName, req.detector,
        // req.descriptor);
        ...
        ROS_FATAL("only the descriptors below are implemented [pfh|fpfh|vfh|rsd|sc3d|usc|shot]");
    }
