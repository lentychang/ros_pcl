...
//#include <pub_msgs/pubMsgType.h>             // [TODO] rename: pub_msgs, pubMsgType
...
{
  public:
    RecogSrv(ros::NodeHandle *nodehandle); //"main" will need to instantiate a ROS nodehandle, then pass it to the constructor
...
RecogSrv::RecogSrv(ros::NodeHandle *nodehandle) : nh_(*nodehandle), __rate(5)
{ // constructor
...
void RecogSrv::initializeServices()
{
    ...
    __service = nh_.advertiseService("exampleMinimalService",
                                     &RecogSrv::serviceCallback,
                                     this);
...
void RecogSrv::initializePublishers()
{
    ...
    // note: COULD make __publisher a public member function, if want to use it within "main()"
}
...
int main(int argc, char **argv)
{
    ...
    RecogSrv RecogSrv(&nh); //instantiate an RecogSrv object and pass in pointer to nodehandle for constructor to use
